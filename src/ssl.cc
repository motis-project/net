...
ssl::ssl(boost::asio::io_service& io_service,
...
void ssl::connect(connect_cb cb) {
  req_timeout_timer_.async_wait(std::bind(&ssl::timer_callback, this,
                                          shared_from_this(),
                                          std::placeholders::_1));
...
void ssl::resolve(ssl_ptr self, connect_cb cb) {
  ...
  return resolver_.async_resolve(query, std::bind(&ssl::on_resolve, this,
                                                  std::move(self),
                                                  std::move(cb),
                                                  std::placeholders::_1,
                                                  std::placeholders::_2));
...
void ssl::on_resolve(
  ...
  if (!ec) {
    return boost::asio::async_connect(socket_.lowest_layer(), iterator,
                                      std::bind(&ssl::on_connect, this,
                                               std::move(self),
                                               std::move(cb),
                                               std::placeholders::_1,
                                               std::placeholders::_2));
...
void ssl::on_connect(
  ...
  if (!ec) {
    return socket_.async_handshake(boost::asio::ssl::stream_base::client,
                                   std::bind(&ssl::on_handshake, this,
                                             self, cb,
                                             std::placeholders::_1));
  } else {
    finally(ec);
